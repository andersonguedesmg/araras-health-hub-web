...
export class ReceivingService {
  ...
  public loadReceivings(
    pageNumber: number,
    pageSize: number,
    searchTerm: string = '',
  ): Observable<ApiResponse<Receiving[]>> {
    ...
    return this.http.put<ApiResponse<Receiving>>(url, receiving).pipe(
      tap((response) => {
        if (response.success && response.data) {
          ...
          const updatedList = currentProducts.map((r) =>
            r.id === receivingId ? response.data! : r,
          );
