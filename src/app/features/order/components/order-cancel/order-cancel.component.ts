...
export class OrderCancelComponent extends BaseComponent implements OnInit, OnDestroy {
  FormMode = FormMode;
  ConfirmMode = ConfirmMode;
  statusOptions = StatusOptions;

  itemsBreadcrumb: MenuItem[] = [{ label: 'Pedidos' }, { label: 'Cancelados' }];
  title: string = 'Pedidos Cancelados';
  description: string = '';
  ...
  async exportOrders(): Promise<void> {
    await this.exportData(
      (searchTerm) => this.orderService.exportOrders(searchTerm, OrderStatusId.Cancelled),
      'pedido-cancelado.csv',
      this.searchTerm
    );
  }
}
